...
describe('FileDropZoneComponent:', () => {
    ...
    let spectatorHost: SpectatorHost<SuT>;
    let sut: SuT;
    ...
    describe('drop zone without input', () => {
        beforeEach(() => {
            spectatorHost = createComponentHost(`
                <div #wrapperDropZone
                     style="width: 100px;height: 100px;"
                     data-testid="custom-drop-zone">
                    <ui-file-drop-zone [customDropZone]="wrapperDropZone"
            ...
            fileReaderService = spectatorHost.inject(FileReaderService, true);
            spyOnProperty(fileReaderService, 'files$').and.returnValue(filesMock$);
            spyOnProperty(fileReaderService, 'error$').and.returnValue(errorMock$);

            spectatorHost.detectChanges();
            sut = spectatorHost.component;
        ...
        it('should add class to drop zone when dragging over', () => {
            ...
            spectatorHost.dispatchMouseEvent(byTestId('file-input'), 'dragover', undefined, undefined, mouseEvent);
            spectatorHost.detectChanges();

            expect(spectatorHost.queryHost(byTestId('custom-drop-zone'))).toHaveClass('file-drop-zone-highlight');
        });

        it('should call service with files that were dropped', () => {
            const mouseEvent = createMouseEvent('drop');

            spectatorHost.dispatchMouseEvent(byTestId('file-input'), 'drop', undefined, undefined, mouseEvent);
            spectatorHost.detectChanges();
